...
static const std::unordered_set<uint32_t> KNOWN_ASSETS_SET
{
    // images
    "bmp"_hash, "png"_hash,
...
private:
    ...
    int _open(cef_request_t* request, int* handle_request, cef_callback_t* callback)
    {
        ...
        std::u16string fpath, query_part;
        fpath.assign((char16_t *)url.str + 15, url.length - 15); // skip 'https://plugins'

        // Check query part.
        if ((pos = fpath.rfind('?')) != std::u16string::npos)
        {
            query_part = fpath.substr(pos + 1);
        ...
        // Get final path.
        fpath = config::plugins_dir().u16string().append(fpath);

        // Trailing slash.
        if (fpath[fpath.length() - 1] == '/' || fpath[fpath.length() - 1] == '\\')
        {
            js_mime = true;
            fpath.append(u"index.js");
        }
        else
        {
            size_t sep_pos = fpath.find_last_of(u"//\\");
            std::u16string sub = fpath.substr(sep_pos + 1);

            // No extension.
            if (sub.rfind('.') == std::u16string::npos)
            {
                // peek .js
                if ((js_mime = file::is_file(fpath + u".js")))
                    fpath.append(u".js");
                // peek folder
                else if ((js_mime = file::is_dir(fpath)))
                    fpath.append(u"/index.js");
            }
        }
        ...
        if (file::is_file(fpath))
        {
            ...
            if (request->get_resource_type(request) == RT_SCRIPT)
            {
                ...
                else if ((pos = fpath.rfind('.')) != std::u16string::npos)
                {
            ...
            {
                stream_ = cef_stream_reader_create_for_file(&CefStr::wrap(fpath));
            }
        }
        ...
        if (stream_ != nullptr)
        {
            ...
            else if ((pos = fpath.rfind(u'.')) != std::u16string::npos)
            {
                auto ext = fpath.substr(pos + 1);
